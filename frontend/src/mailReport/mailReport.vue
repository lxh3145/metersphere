<template>
    <div class="mailReportMain">
      <el-tabs v-model="activeName" @tab-click="handleClick">
        <el-tab-pane :label="$t('api_report.total')" name="total">
          <full-report :fullDataList="item" v-for="(item,index) in fullDataAllList" :key="index" />
        </el-tab-pane>
        <el-tab-pane name="fail">
          <template slot="label">
            <span class="fail">{{ $t('api_report.fail') }}</span>
          </template>
          <fails-report :fullDataList="item" v-for="(item,index) in fullDataList" :key="index" />
        </el-tab-pane>
        <el-tab-pane name="fail_detail">
          <template slot="label">
            <span class="fail_detail">{{ $t('api_report.fail_detail') }}</span>
          </template>
          <fails-detail-report :fullDataList="item" v-for="(item,index) in fullDataList" :key="index" />
        </el-tab-pane>
      </el-tabs>
    </div>
</template>
...
import MsScenarioResults from "@/business/components/api/automation/report/components/ScenarioResults";
import {RequestFactory} from "@/business/components/api/definition/model/ApiTestModel";
import FullReport from "./fullReport";
import FailsReport from "./failsReport";
import FailsDetailReport from "./failsDetailReport";
...
export default {
  ...
  methods: {
    ...
    getReport() {
      this.init();
      if (this.reportId) {
        let url = "/api/scenario/report/getWrongReportList/" + this.reportId;
        //let url = "/api/scenario/report/get/" + this.reportId;
        this.$get(url, response => {
          this.report = response.data || {};
          this.fullDataList = response.data || [];
          if (response.data) {
        ...
        let AllUrl = "/api/scenario/report/getAllReportList/" + this.reportId;
        this.$get(AllUrl, response => {
          this.allReport = response.data || {};
          this.fullDataAllList = response.data || [];
        });
      }
    },
    getFails() {
  ...
  created() {
    this.$nextTick(() => {
      if(window.location.pathname == '/mailReport'){
        this.reportId = window.location.search.substring(10);
      }
    })
    this.getReport();
  },
...
<style scoped>
.mailReportMain .el-tabs.el-tabs--top{
  padding: 0 24px;
}
.report-container {
